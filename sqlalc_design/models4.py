...
class TransactionMaster(Base):
    ...
    __table_args__ = (
        CheckConstraint("transaction_type::text = ANY (ARRAY['Internal'::text, 'Financial'::text, 'Non-Financial'::text]::text[])",
                        name='txn_type_check')
        # Index('idx_transaction_type_reference', 'transaction_type')
    )
...
class TransactionBranchMapping(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_transaction_branch_mapping_reference', 'branch_id', 'transaction_code')
    )
...
class Session(Base):
    ...
    customer_rep: Mapped[str] = mapped_column(String(1), nullable=False)
    ...
    account_no: Mapped[dict] = mapped_column(JSONB, nullable=False)
    ...
    representative_data: Mapped[dict] = mapped_column(JSONB, nullable=True)  #{id_no, first_name, last_name, dob, gender}

    __table_args__ = (
        ForeignKeyConstraint(['branch_id'], ['branch_master.branch_id'],
                             name='session_fk_branch_id'),
        CheckConstraint("customer_rep::text = ANY (ARRAY['C'::character varying, 'R'::character varying]::text[])",
                        name='customer_rep_check'),
        CheckConstraint("authentication_method::text = ANY (ARRAY['Card'::text, 'CIF'::text, 'Phone Number'::text, 'Account number'::text, 'NID'::text, 'Passport'::text, 'DL'::text]::text[])",
...
class CurrencyBranchMapping(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_currency_branch_mapping_reference', 'branch_id', 'currency_code')
...
class DenominationBranchMapping(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_denomination_branch_mapping_reference', 'branch_id', 'currency_code')
    )
...
class Transactions(Base):
    ...
    transaction_code: Mapped[str] = mapped_column(String(50), nullable=False)
    transaction_status: Mapped[Optional[str]] = mapped_column(String(1), nullable=False)
    auth_status: Mapped[Optional[str]] = mapped_column(String(1), nullable=False)
    session_id: Mapped[str] = mapped_column(String(36))
    created_at_branch_id: Mapped[str] = mapped_column(String(50))
    present_at_branch_id: Mapped[str] = mapped_column(String(50))
    created_by_username: Mapped[str] = mapped_column(String(255))
    created_by_user_id: Mapped[str] = mapped_column(String(50))
    approver_provider: Mapped[str] = mapped_column(String(50), nullable=False)
    create_timestamp: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True))
    last_updated_username: Mapped[str] = mapped_column(String(255))
    last_updated_user_id: Mapped[str] = mapped_column(String(50))
    last_updated_approver_provider: Mapped[str] = mapped_column(String(50))
    last_updated_timestamp: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True))
    screen_data: Mapped[dict] = mapped_column(JSONB)
    ...
    __table_args__ = (
        ...
        Index('idex_transactions_screen_data_gin', 'screen_data', postgresql_using = 'gin'),
        Index('idex_transactions_additional_data_gin', 'additional_data', postgresql_using = 'gin')
    )
...
class TransactionAuditLog(Base):
    ...
    old_json: Mapped[dict] = mapped_column(JSONB)
    new_json: Mapped[dict] = mapped_column(JSONB)
    ...
    action_by_user_id: Mapped[str] = mapped_column(String(255), nullable=False)
    ...
    __table_args__ = (
        ...
        Index('idex_transaction_audit_new_json_gin', 'new_json', postgresql_using = 'gin')
    )

class TillMaster(Base):
    __tablename__ = 'till_master'
    branch_id: Mapped[str] = mapped_column(String(50), nullable=False)
    till_id: Mapped[str] = mapped_column(String(50), nullable=False)
    currency_code: Mapped[str] = mapped_column(String(3), nullable=False)
    current_denominations: Mapped[dict] = mapped_column(JSONB)
    current_balance: Mapped[int] = mapped_column(Integer)
    opening_denominations: Mapped[dict] = mapped_column(JSONB)
...
class TillTypeMapping(Base):
    ...
    till_id: Mapped[str] = mapped_column(String(50), primary_key=True)
...
class UserTillMaster(Base):
    ...
    username: Mapped[str] = mapped_column(String(255), nullable=False)
    till_id: Mapped[str] = mapped_column(String(50), unique=True)
    aprover_provider: Mapped[str] = mapped_column(String(50))
    aprover_id: Mapped[str] = mapped_column(String(50), nullable=False)
    aprover_name: Mapped[str] = mapped_column(String(255))
    ...
    __table_args__ = (
        ...
        Index('idx_user_till_reference', 'till_id')
    )
...
class TransactionDenomination(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(['transaction_reference_no', 'till_id', 'currency_code'], name='transaction_denomination_pkey'),
        ForeignKeyConstraint(['transaction_reference_no'], ['transactions.transaction_reference_no'],
